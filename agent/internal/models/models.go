...
type Node struct {
	...
	Endpoint    string    `json:"endpoint"` // Last known WireGuard endpoint (IP:port)
	LastSeenAt  time.Time `json:"last_seen_at"`
	CreatedAt   time.Time `json:"created_at"`
}

// Peer represents another node in the same network.
type Peer struct {
	PublicKey   string `json:"public_key"`
	IPv4Address string `json:"ipv4_address"`
...
// NodeConfigResponse is returned to the agent during sync.
